...
public class Field {
	//Data: Cell容器，传给View
	/*Field与Cell关系：Cell在Field中，但Cell很多操作需要Field数据
	1）让每个Cell有一个Field管理者（Cell知道Field）
	2）由外部（FoxAndRabbit）建立两者联系（Cell不知道Field）
	* */
	private int width;
	private int height;
...
//		System.out.println(list.toArray(new Cell[list.size()]));
//		System.out.println(list.toArray(new Cell[list.size()]).getClass());
